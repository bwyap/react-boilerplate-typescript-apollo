import { Reducer, Store } from 'redux';
import {
  ReduxStore,
  Saga,
  SagaDescriptor,
  SagaInjectionHandlers,
} from '../types';

type CheckStoreFn = (store: ReduxStore<any, any>) => void;

type ReducerInjectorFn = (
  reducerInjection: ReducerInjection,
) => (
  WrapperComponent: React.ComponentClass | React.FunctionComponent,
) => React.ComponentClass;

interface ReducerInjection {
  key: string;
  reducer: Reducer;
}

type SagaInjectorFn = (
  sagaInjection: SagaInjection,
) => (
  WrapperComponent: React.ComponentClass | React.FunctionComponent,
) => React.ComponentClass;

interface SagaInjection {
  key: string;
  saga: Saga;
  mode?: string;
}

type InjectReducerFactory = (
  store: ReduxStore<any, any>,
  isValid?: boolean,
) => InjectReducerFn;

type InjectReducerFn = (key: string, reducer: Reducer) => void;

type GetReducerInjectorsFn = (
  store: ReduxStore,
) => { injectReducer: InjectReducerFn };

type InjectSagaFactory = (store: ReduxStore, isValid?: boolean) => InjectSagaFn;

type InjectSagaFn = (
  key: string,
  descriptor: SagaDescriptor,
  args?: object,
) => void;

type EjectSagaFactory = (store: ReduxStore, isValid?: boolean) => EjectSagaFn;

type EjectSagaFn = (key: string) => void;

type GetSagaInjectorsFn = (store: ReduxStore) => SagaInjectionHandlers;
